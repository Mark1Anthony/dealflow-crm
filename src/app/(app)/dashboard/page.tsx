import { getDashboardStats, getRecentActivities } from "@/lib/queries/dashboard";
import { getDealsByStage } from "@/lib/queries/deals";
import { StatCard } from "@/components/StatCard";
import { ActivityFeed } from "@/components/ActivityFeed";
import { TopBar } from "@/components/TopBar";
import { SeedButton } from "@/components/SeedButton";
import { formatCurrency } from "@/lib/utils";

export default async function DashboardPage() {
  const [stats, activities, stageData] = await Promise.all([
    getDashboardStats(),
    getRecentActivities(8),
    getDealsByStage(),
  ]);

  const isEmpty = stats.totalContacts === 0 && stats.activeDeals === 0;

  return (
    <>
      <TopBar title="Dashboard" subtitle="Business overview" />

      {isEmpty && (
        <div className="bg-cyan-500/5 border border-cyan-500/15 rounded-2xl p-8 mb-8 text-center">
          <div className="w-14 h-14 mx-auto rounded-full bg-cyan-500/10 flex items-center justify-center mb-4">
            <svg className="w-7 h-7 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>
          </div>
          <h3 className="text-lg font-bold text-zinc-100 mb-2">Welcome to DealFlow</h3>
          <p className="text-zinc-400 text-sm mb-5 max-w-md mx-auto">
            Your CRM is empty. Load demo data to see what the app looks like with contacts, deals and activities, or start from scratch.
          </p>
          <SeedButton />
        </div>
      )}

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          title="Contacts"
          value={stats.totalContacts.toString()}
          icon={<svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" /></svg>}
        />
        <StatCard
          title="Active deals"
          value={stats.activeDeals.toString()}
          icon={<svg className="w-4 h-4 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" /></svg>}
        />
        <StatCard
          title="Pipeline value"
          value={formatCurrency(stats.pipelineValue)}
          icon={<svg className="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
        />
        <StatCard
          title="Activities (7d)"
          value={stats.activitiesThisWeek.toString()}
          icon={<svg className="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>}
        />
      </div>

      {/* Pipeline summary + Activity feed */}
      <div className="grid lg:grid-cols-[2fr,1fr] gap-4">
        <div className="bg-[#111218] border border-white/5 rounded-2xl p-5">
          <h2 className="text-zinc-50 font-bold mb-4">Pipeline</h2>
          {stageData.length === 0 ? (
            <p className="text-zinc-500 text-sm">No pipeline stages yet. Load demo data or create stages in Settings.</p>
          ) : (
            <div className="space-y-3">
              {stageData.map(({ stage, deals }) => (
                <div key={stage.id} className="flex items-center gap-3">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: stage.color || "#666" }} />
                  <div className="flex-1 text-sm text-zinc-300">{stage.name}</div>
                  <div className="text-sm font-semibold text-zinc-100">{deals.length}</div>
                  <div className="text-xs text-zinc-500">{formatCurrency(deals.reduce((s, d) => s + (d.value || 0), 0))}</div>
                </div>
              ))}
            </div>
          )}
        </div>

        <div className="bg-[#111218] border border-white/5 rounded-2xl p-5">
          <h2 className="text-zinc-50 font-bold mb-4">Recent activity</h2>
          {activities.length === 0 ? (
            <p className="text-zinc-500 text-sm">No activities yet.</p>
          ) : (
            <ActivityFeed activities={activities} />
          )}
        </div>
      </div>
    </>
  );
}
